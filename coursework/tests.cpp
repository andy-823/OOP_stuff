...
void TestGet(std::size_t to)
{
    ...
    segment_tree<int, int> tree1;
    tree1.build_tree(v, sum);

    lazy_tree<int, int> tree2;
    tree2.build_tree(v, sum);
    ...
    for (int j = i; j < to; j++)
    {
        ...
        try
        {
            ...
            for (int k = i; k <= j; k++)
                    res_correct += v[k];
            res_first = tree1.get_answer(i, j);
            res_second = tree2.get_answer(i, j);
...
void testUpdate(int to)
{
    ...
    for (int i = 0; i < to; i++)
    {
        ...
        tree1.update_tree(v2, first, second);
        tree2.update_tree(v2, first, second);
...
void testHard(int to)
{
    ...
    segment_tree<std::vector<int>, hard_type> tree1;
    tree1.build_tree(v, compare, len);

    lazy_tree<std::vector<int>, hard_type> tree2;
    tree2.build_tree(v, compare, len);
